...
export async function POST(
  ...
  try {
    ...
    if (invoice.poster_id !== user.id) {
      return NextResponse.json({ error: "Only the poster can pay this invoice" }, { status: 403 });
    }
    ...
    const paymentCurrency = await resolveSupportedPaymentCurrency(gig?.payment_coin, {
