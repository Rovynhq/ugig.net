...
export function registerWalletCommands(program: Command): void {
  ...
  addresses
    ...
    .action(async () => {
      ...
      try {
        ...
        spinner?.stop();
        const addrs = (result as any).poster_addresses ?? [];
        if (addrs.length === 0 && !opts.json) {
          console.log(
            "No wallet addresses saved. Run `ugig coinpay import` to import from CoinPay."
          );
          return;
        }
        printTable(
  ...
  addresses
    ...
    .action(async (currency: string, address: string, cmdOpts: { preferred?: boolean }) => {
      const opts = program.opts() as GlobalOpts;
      ...
      try {
        ...
        const existing: { currency: string; address: string; is_preferred: boolean }[] =
          (current as any).poster_addresses ?? [];
        ...
        spinner?.stop();
        printSuccess(`Saved ${currency} address to profile`, opts as OutputOptions);
      } catch (err) {
        spinner?.fail("Failed");
        handleError(err, opts as OutputOptions);
      }
  ...
  addresses
    ...
    .action(async (currency: string) => {
      ...
      try {
        ...
        spinner?.stop();
        printSuccess(`Removed ${currency} address from profile`, opts as OutputOptions);
      } catch (err) {
        spinner?.fail("Failed");
        handleError(err, opts as OutputOptions);
      }
  ...
  addresses
    ...
    .action(async (cmdOpts: { merge?: boolean }) => {
      ...
      try {
        const client = createClient(opts);
        const result = await client.get<any>("/api/coinpay/wallets");
        const data = result?.data ?? result;

        if (data.oauth_required) {
          spinner?.fail("Not connected");
          console.error(
            "CoinPay OAuth not connected. Connect at Settings > Connections in the ugig web app."
          );
          process.exitCode = 1;
          return;
        }
        ...
        const incoming = data.wallets.map((w: any, i: number) => ({
          currency: w.currency,
          address: w.address,
          is_preferred: i === 0,
        }));
        ...
        let toSave = incoming;
        if (cmdOpts.merge) {
          const current = await client.get<any>("/api/profile/wallet-addresses");
          const existing = current?.poster_addresses ?? [];
          const merged = [...existing];
          for (const addr of incoming) {
            const idx = merged.findIndex((a: any) => a.currency === addr.currency);
            if (idx >= 0) merged[idx] = addr;
            else merged.push(addr);
          }
